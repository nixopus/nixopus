...
class Install:
    def __init__(
        ...
        # Load config and create params
        self.config = get_active_config(user_config_file=self.config_file)
        self.params = InstallParams(
            ...
            caddy_admin_port=self.caddy_admin_port,
            caddy_http_port=self.caddy_http_port,
            caddy_https_port=self.caddy_https_port,
            supertokens_port=self.supertokens_port,
            external_db_url=self.external_db_url,
            staging=self.staging,
        )
        
        # Validate parameters
        validate_install_params(self.params)
        
        # Log when using custom repository/branch and staging compose file
        if is_custom_repo_or_branch(self.repo, self.branch):
            if self.logger:
    ...
    def run(self):
        config_resolver = create_config_resolver(self.config, self.params)
        steps = build_installation_steps(self.config, config_resolver, self.params)
        
        # Always load proxy configuration after services are started
        steps.append(("Loading proxy configuration", lambda: load_proxy_step(config_resolver, self.params)))
        ...
        try:
            ...
            show_success_message(config_resolver, self.params)

        except Exception as e:
            handle_installation_error(e, self.params)
            if self.logger:
