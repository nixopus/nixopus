...
func TestAddCustomDomain_NoOrgID(t *testing.T) {
	...
	Test(t,
		Description("POST /domain/custom without X-Organization-ID uses session org; missing infra returns 500"),
...
func TestGenerateRandomSubdomain_NoOrgID(t *testing.T) {
	...
	Test(t,
		Description("GET /domain/generate without X-Organization-ID uses session org; no domain pool returns 404"),
		Get(tests.GetDomainGenerateURL()),
		Send().Headers("Cookie").Add(auth.GetAuthCookiesHeader()),
...
func TestRemoveCustomDomain_NoOrgID(t *testing.T) {
	...
	Test(t,
		Description("DELETE /domain/custom without X-Organization-ID uses session org; unknown id returns 404"),
		Delete(tests.GetDomainCustomURL()),
		Send().Headers("Cookie").Add(auth.GetAuthCookiesHeader()),
		Send().Body().JSON(map[string]interface{}{"id": uuid.New().String()}),
		Expect().Status().Equal(http.StatusNotFound),
...
func TestVerifyCustomDomain_NoOrgID(t *testing.T) {
	...
	Test(t,
		Description("POST /domain/verify without X-Organization-ID uses session org; unknown id returns 404"),
		Post(tests.GetDomainVerifyURL()),
		Send().Headers("Cookie").Add(auth.GetAuthCookiesHeader()),
		Send().Body().JSON(map[string]interface{}{"id": uuid.New().String()}),
		Expect().Status().Equal(http.StatusNotFound),
...
func TestCheckDNSStatus_NoOrgID(t *testing.T) {
	...
	Test(t,
		Description("GET /domain/dns-check without X-Organization-ID uses session org; unknown id returns 404"),
		Get(tests.GetDomainDNSCheckURL(uuid.New().String())),
		Send().Headers("Cookie").Add(auth.GetAuthCookiesHeader()),
		Expect().Status().Equal(http.StatusNotFound),
	)
