...
func newTestUserDB(t *testing.T) (*bun.DB, context.Context) {
	...
	// UpdateUserName and UpdateUserAvatar target the legacy "users" table
...
func TestUpdateUserName_success(t *testing.T) {
	...
	id := uuid.New()
	_, err := db.ExecContext(ctx,
		`INSERT INTO users (id, username, updated_at) VALUES (?, ?, CURRENT_TIMESTAMP)`,
		id.String(), "oldname",
	)
	require.NoError(t, err)
	...
	var username string
	err = db.NewRaw("SELECT username FROM users WHERE id = ?", id.String()).Scan(ctx, &username)
	require.NoError(t, err)
	assert.Equal(t, "newname", username)
}
