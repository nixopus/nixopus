...
func TestSendOTP_marshalError(t *testing.T) {
	prev := authJSONMarshal
	authJSONMarshal = func(v any) ([]byte, error) {
		return nil, errors.New("marshal")
	}
	t.Cleanup(func() { authJSONMarshal = prev })
...
func TestSendOTP_newRequestFailure(t *testing.T) {
	revert := saveBetterAuth(types.BetterAuthConfig{URL: "http://[\n", Secret: "s"})
	t.Cleanup(revert)
...
func TestSendOTP_httpFails(t *testing.T) {
	revertBa := saveBetterAuth(types.BetterAuthConfig{URL: "http://127.0.0.1:1", Secret: "s"})
	t.Cleanup(revertBa)
	revertHc := saveHTTPClient(&http.Client{Transport: errRoundTripper{err: errors.New("down")}})
	t.Cleanup(revertHc)
	...
	err := SendOTP("a@b")
	require.Error(t, err)
	require.Contains(t, err.Error(), "send OTP")
}

func TestSendOTP_nonOK(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
...
func TestSendOTP_ok(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		require.Equal(t, "/api/auth/email-otp/send-verification-otp", r.URL.Path)
		b, _ := io.ReadAll(r.Body)
		require.Contains(t, string(b), "a@b")
		w.WriteHeader(http.StatusOK)
	}))
	t.Cleanup(srv.Close)
